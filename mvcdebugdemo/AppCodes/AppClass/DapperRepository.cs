...
public class DapperRepository : BaseClass
{
    ...
    /// <returns></returns>
    public bool CheckConnectionName(string connectionName)
    {
        return CheckConnectionString(GetConnectionString(connectionName));
    }
    /// <summary>
    ...
    /// <param name="pageSize">筆頁筆數</param>
    /// <returns></returns>
    public async Task<X.PagedList.IPagedList<T>> ReadAllPageAsync<T>(string query, int page, int pageSize)
    {
        ErrorMessage = "";
        List<T> ReturnValue = new List<T>();
        using var conn = new SqlConnection(ConnectionString);
        try
        {
            conn.Open();
            var data = await conn.QueryAsync<T>(query);
            return data.ToList().ToPagedList(page, pageSize);
        }
        catch (Exception ex)
        {
            ErrorMessage = ex.Message;
        ...
        return ReturnValue.ToPagedList();
    }
    /// <summary>
    /// 讀取多筆記錄(非同步)
    /// </summary>
    ...
    /// <param name="pageSize">筆頁筆數</param>
    /// <returns></returns>
    public async Task<X.PagedList.IPagedList<T>> ReadAllPageAsync<T>(string query, DynamicParameters parameters, int page, int pageSize)
    {
        ErrorMessage = "";
        List<T> ReturnValue = new List<T>();
        using var conn = new SqlConnection(ConnectionString);
    ...
    /// <param name="query">命令字串</param>
    /// <returns></returns>
    public GridReader ReadMulti(string query)
    {
        ...
        {
            conn.Open();
            multi = conn.QueryMultiple(query);
        }
        catch (Exception ex)
        {
            ErrorMessage = ex.Message;
    ...
    /// <param name="parameters">參數變數物件</param>
    /// <returns></returns>
    public GridReader ReadMulti(string query, DynamicParameters parameters)
    {
        GridReader multi = null;
        using var conn = new SqlConnection(ConnectionString);
        try
        {
            conn.Open();
            multi = conn.QueryMultiple(query, parameters);
        }
        catch (Exception ex)
        {
            ErrorMessage = ex.Message;
    ...
    /// <param name="query">命令字串</param>
    /// <returns></returns>
    public async Task<GridReader> ReadMultiAsync(string query)
    {
        GridReader multi = null;
        using var conn = new SqlConnection(ConnectionString);
        try
        {
            conn.Open();
            multi = await conn.QueryMultipleAsync(query);
        }
        catch (Exception ex)
        {
            ErrorMessage = ex.Message;
    ...
    /// <param name="parameters">參數變數物件</param>
    /// <returns></returns>
    public async Task<GridReader> ReadMultiAsync(string query, DynamicParameters parameters)
    {
        GridReader multi = null;
        using var conn = new SqlConnection(ConnectionString);
        try
        {
            conn.Open();
            multi = await conn.QueryMultipleAsync(query, parameters);
        }
        catch (Exception ex)
        {
            ErrorMessage = ex.Message;
    ...
    /// <returns></returns>
    public string GetSQLSelectCommand<TEntity>(TEntity entity, List<string> columns, string sqlWhere = "", string sqlOrderBy = "")
        ...
        else
        {
            foreach (var col in columns)
            {
                ...
                if (col.Contains("."))
                    ...
                    str_command += $"{str_entity_name}.{col} ";
            }
        }
        str_command += $" FROM {str_entity_name} ";
        if (!string.IsNullOrEmpty(sqlWhere)) str_command += $" WHERE {sqlWhere} ";
        if (!string.IsNullOrEmpty(sqlOrderBy)) str_command += $" ORDER BY {sqlOrderBy} ";
        return str_command;
    }
    /// <summary>
    /// 動態取得 SQL Select 語法 (不含 Join 的簡單語法)
    ...
    #endregion
    #region 動態取得 SQL Insert/Delete/Update 語法
    /// <summary>
    /// 動態取得 SQL Insert 語法
    /// </summary>
    /// <typeparam name="T">型別</typeparam>
    ...
    /// <returns></returns>
    public string GetDuplicateMessage<T>(T model, string colName, string metaName = "z_meta")
    {
        using var dmService = new DataModelService();
        string str_namespace = model.GetType().Namespace;
        string str_class = model.GetType().Name;
        string str_metadata = $"{metaName}{str_class}";
        enDataModelProperty propType = enDataModelProperty.DisplayName;
        string str_text = dmService.GetPropertyTypeValue(str_class, colName, propType, str_namespace, str_metadata);
        string str_message = $"{str_text}重覆輸入!!";
        return str_message;
    }
    /// <summary>
    /// 取得 Key 欄位名稱
